...
import type { Board, Card, ColumnPatch, FilterOp, Snapshot } from './model';
...
export const api = {
  ...
  setColumnConfig: (boardId: string, property: string, value: string, patch: ColumnPatch) =>
    invoke<void>('set_column_config', { boardId, property, value, patch }),
  addColumn: (boardId: string, property: string, value: string, color: string, order: number) =>
    invoke<void>('add_column', { boardId, property, value, color, order }),
  reorderColumn: (boardId: string, property: string, value: string, dir: 'left' | 'right') =>
    invoke<void>('reorder_column', { boardId, property, value, dir }),
  reorderColumns: (boardId: string, property: string, order: string[]) =>
    invoke<void>('reorder_columns', { boardId, property, order }),
  renameColumn: (prop: string, from: string, to: string) =>
    invoke<void>('rename_column', { prop, from, to }),
  removeColumn: (boardId: string, property: string, value: string) =>
    invoke<void>('remove_column', { boardId, property, value }),
};
