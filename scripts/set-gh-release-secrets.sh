#!/usr/bin/env bash
# Push GitHub Actions release secrets for the Tauri release pipeline onto a
...
# not an error. RELEASE_REPO_TOKEN is only needed if the workflow mirrors the
# build to a central releases repo (a PAT with `contents: write` there).
OPTIONAL_SECRETS=(
    RELEASE_REPO_TOKEN
)
